...
export default function MetalLaserCutting() {
  const t = useTranslations('MetalLaserCutting');
  ...
  return (
    <main>
      <Hero 
        title={t('heroTitle')} 
        imageSrc="/hero-metal-laster-cutting.png"
        showForm={false}
      />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            {allImages.slice(0, visibleCount).map((i) => (
              <div key={i} className="group relative aspect-video overflow-hidden rounded-xl bg-gray-100">
                <Image 
                  src={`/portfolio/metal-laser-cutting-${i}.png`} 
                  alt={`Metal Laser Cutting ${i}`} 
                  fill 
